...
class RepositoryRoomImpl:RepositoryWeatherByCityLoadable,RepositoryWeatherSavable,RepositoryWeatherAvailable {
    ...
    override fun addWeather(weather: WeatherItem) {
        convertWeatherToEntity(weather)?.let {weatherEntity ->
            WeatherApp.getWeatherDatabase().weatherDao().insertRoom(
                weatherEntity
            )
        }
    }
    ...
    private fun convertWeatherToEntity(weather: WeatherItem): WeatherEntity? {
        return weather.city?.name?.let {
            WeatherEntity(0,
                it, weather.city.lat,weather.city.lon, weather.temperature, weather.pressure, weather.humidity, weather.wind, weather.feelsLike)
        }
    }
